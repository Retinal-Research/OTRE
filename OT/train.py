...
def main():
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        ...
        for i in num_random:
            ...
            a,b,c,d=train(training_data_loader, G_optimizer, D_optimizer, model, discr, epoch,num_rand)
            ot += a
            Gloss+=b
            Gidentity +=c
            num_rand += 1
        ot = ot / len(num_random)
    ...
    file = open('SottGan/Experiment/exp11/checksample/OTloss_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt','w')
    for ot in OT_CONSTRAN:
        file.write(ot+'\n')
    file.close()

    file = open('SottGan/Experiment/exp11/checksample/Gloss_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt', 'w')
    for g in GLOSS:
        file.write(g + '\n')
    file.close()

    file = open('SottGan/Experiment/exp11/checksample/Gidentity_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt', 'w')
    for g in GI:
        file.write(g + '\n')
    file.close()

    file = open('SottGan/Experiment/exp11/checksample/PSNR_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt', 'w')
    for g in Psnr:
        file.write(g + '\n')
    file.close()
    # psnr = eval_dep(model)
...
def train(training_data_loader, G_optimizer, D_optimizer, model, discr, epoch,num_rand):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        #G_train_loss = - D_result.mean() + opt.sigma * ot_loss + 10 * G_identity + 20*G_l
        G_train_loss = - D_result.mean() + opt.sigma * ot_loss + 15 * G_identity 
        ...
        if iteration % 10 == 0:
            #print("===> Epoch[{}]({}/{}): Loss_G: {:.5}, Loss_mse: {:.5}, loss_identity: {:.5}, loss_L1: {:.5}".format(epoch, iteration, len(training_data_loader), G_train_loss.data, ot_loss.data,G_identity.data,G_l.data))
            print("===> Epoch[{}]({}/{}): Loss_G: {:.5}, Loss_mse: {:.5}, loss_identity: {:.5}".format(epoch, iteration, len(training_data_loader), G_train_loss.data, ot_loss.data,G_identity.data))
    save_image(G_result.data, 'SottGan/Experiment/exp11/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'output.png')
    save_image(input.data, 'SottGan/Experiment/exp11/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'input.png')
    save_image(target.data, 'SottGan/Experiment/exp11/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'gt.png')
...
def save_checkpoint(model, discr, epoch):
    model_out_path = "SottGan/Experiment/exp11/checkpoint/" + "model_denoise_"+str(epoch)+"_"+str(opt.sigma)+".pth"
    state = {"epoch": epoch ,"model": model, "discr": discr}
    if not os.path.exists("SottGan/Experiment/exp11/checkpoint/"):
        os.makedirs("SottGan/Experiment/exp11/checkpoint/")
