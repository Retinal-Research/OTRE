...
# Training settings
parser = argparse.ArgumentParser(description="PyTorch RED") 
...
def main():
    ...
    file = open('SottGan/Experiment/exp_full_5/checksample/ot_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt','w')
...
def train(training_data_loader, G_optimizer, D_optimizer, model, discr, epoch,num_rand):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        # ot_loss = (torch.mean((G_result- input)**2))**0.5
        ...
        #G_train_loss = - D_result.mean() + opt.sigma * ot_loss +  opt.lamda1 * G_identity +  opt.lamda2 *G_l
        G_train_loss = - D_result.mean() + opt.sigma * ot_loss +  opt.lamda1* G_identity
        ...
        if iteration % 10 == 0:
            ...
            #print("===> Epoch[{}]({}/{}): Loss_G: {:.5}, Loss_mse: {:.5}, loss_identity: {:.5}, loss_L1: {:.5f}".format(epoch, iteration, len(training_data_loader), G_train_loss.data, ot_loss.data,G_identity.data,G_l.data))
